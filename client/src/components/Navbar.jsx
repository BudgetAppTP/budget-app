...
export default function Navbar() {
  ...
  const updateDate = () => {
    ...
    if (window.innerWidth > 1600) {
      ...
      setToday(now.toLocaleDateString(locale));
    }
  };
  ...
  const showBurger = isSmall && !isAuthPage;
  const showMobileMenu = isSmall && !isAuthPage;

  const showLangWide = isWide;
  const showLangDropdown = isMedium || (isSmall && isAuthPage);
    ...
    <>
      <nav className="navbar" aria-label="Top Navigation">
        <div className="nav-inner">
          ...
          {!isSmall && showNavLinks && (
            <div className="nav-links" role="navigation">
              <NavLink to="/" end>
                <T sk="Domov" en="Home" />
              </NavLink>
              <NavLink to="/incomes" onClick={closeMobileMenu}>
                <T sk="Príjmy" en="Incomes" />
              </NavLink>
              <NavLink to="/expenses" onClick={closeMobileMenu}>
                <T sk="Výdavky" en="Expenses" />
              </NavLink>
              <NavLink to="/budgets" onClick={closeMobileMenu}>
                <T sk="Rozpočty" en="Budgets" />
              </NavLink>
              <NavLink to="/savings" onClick={closeMobileMenu}>
                <T sk="Úspory" en="Savings" />
              </NavLink>
              <NavLink to="/needs" onClick={closeMobileMenu}>
                <T sk="Potreby" en="Needs" />
              </NavLink>
              <NavLink to="/organisation" onClick={closeMobileMenu}>
          ...
          <div className="right-bar">
            ...
            <div className="language">
              {showLangWide && (
                <>
                  <button
                    className={`flag-btn ${lang === "sk" ? "active" : ""}`}
                    onClick={() => setLang("sk")}
                  >
              ...
              {showLangDropdown && (
                <>
                  <button
                    className="lang-trigger flag-btn"
