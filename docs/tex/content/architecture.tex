...
Smerovanie API je organizované pomocou jedného nadradeného Flask \textit{Blueprint}, ktorý reprezentuje verejné REST API aplikácie. V rámci tohto blueprintu sú trasy logicky rozdelené do samostatných modulov podľa doménových konceptov, ako sú autentifikácia, príjmy, výdavky, kategórie, rozpočty, sporiace fondy a ciele. Tento hybridný prístup zachováva jednotný API \textit{namespace} a zároveň zabezpečuje modulárnu organizáciu zdrojového kódu, čím vyvažuje jednoduchosť a udržiavateľnosť.
...
\begin{enumerate}
    ...
    s JSON \mbox{payloadom}, ktorý obsahuje polia príjmu. Identita používateľa sa neurčuje z tela požiadavky, ale z autentifikovaného kontextu.
    ...
    s identifikátorom autentifikovaného používateľa (vzťah cudzej väzby na model User).
...
\begin{itemize}
    ...
    \item \textbf{Neplatná alebo chýbajúca autentifikácia:} Ak požiadavka neobsahuje platný
    session token, backend vráti odpoveď \texttt{401 Unauthorized}.
...
API používa jednotnú autentifikačnú kontrolu na úrovni Flask blueprintu. Väčšina koncových bodov vyžaduje platný session token, ktorý môže byť dodaný cez cookie alebo hlavičku \texttt{Authorization}. Po overení je používateľ uložený v \texttt{g.current\_user} a servisná vrstva používa jeho identifikátor pri vytváraní a filtrovaní používateľských zdrojov.

Hodnoty \texttt{user\_id} dodané klientom v tele alebo query parametroch sa pri vlastnených zdrojoch nepoužívajú ako autoritatívny zdroj identity. Tento prístup zabraňuje tomu, aby klient vytváral alebo čítal údaje v mene iného používateľa iba zmenou vstupného payloadu.
