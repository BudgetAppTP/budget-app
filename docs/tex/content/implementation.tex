...
Testovanie a zabezpečenie kvality sú implementované predovšetkým na strane backendu so zameraním na overovanie externe pozorovateľného správania API. Podpora nasadzovania je riešená pomocou Docker Compose a GitHub Actions workflow, pričom produkčné nasadenie je rozdelené na stabilnú infraštruktúru a aplikačné služby.
...
\subsubsection{Nasadenie a kontajnerizácia}

Aplikácia obsahuje lokálne Docker Compose prostredie, ktoré simuluje produkčné nasadenie. Súbor \texttt{docker-compose.yaml} spúšťa databázu PostgreSQL, backend, frontend a externý nginx ako spoločný vstupný bod. Cieľom tohto nastavenia je overiť komunikáciu služieb cez kontajnerovú sieť a priblížiť lokálne správanie produkčnému prostrediu, nie iba spúšťať jednotlivé časti aplikácie izolovane.

Produkčné Compose súbory sú rozdelené podľa životného cyklu služieb:
\begin{itemize}
    \item \path{docker-compose-prod-infra.yml} obsahuje dlhšie žijúcu infraštruktúru, teda PostgreSQL, externý nginx a zdieľanú Docker sieť.
    \item \path{docker-compose-prod-services.yml} obsahuje aplikačné služby backendu a frontendu, ktoré sa pripájajú k existujúcej produkčnej sieti.
\end{itemize}
Toto rozdelenie umožňuje nasadzovať zmeny v aplikácii bez zbytočného zásahu do databázy alebo reverzného proxy.

Automatizácia nasadenia je implementovaná v GitHub Actions:
\begin{itemize}
    \item \path{deploy-prod-infra.yml} sa spúšťa manuálne a umožňuje vybrať, či sa má nasadiť PostgreSQL, nginx alebo obe časti infraštruktúry.
    \item \path{deploy-prod-services.yml} sa spúšťa automaticky pri pushi do vetiev \texttt{main} a \texttt{master} a dá sa spustiť aj manuálne.
\end{itemize}
Pri nasadení služieb sa zostavia Docker image backendu a frontendu, odošlú sa do GitHub Container Registry a následne sa cez SSH aktualizujú kontajnery na cieľovom serveri.

Citlivé hodnoty, ako SSH kľúč, prístupový token do GHCR, databázové heslo, \texttt{SECRET\_KEY} a údaje pre HTTP Basic Auth, sú uložené v GitHub Secrets. Menej citlivé prevádzkové nastavenia, napríklad hostiteľ, používateľ, port SSH, vzdialený adresár alebo názov Docker siete, môžu byť uložené ako GitHub Secrets alebo GitHub Variables.

Nasadzovacie skripty na serveri synchronizujú pracovnú kópiu repozitára na nasadzovanú vetvu alebo tag, prihlásia sa do GHCR a spustia príslušný Compose súbor. Aby sa predišlo zbytočným reštartom, ukladajú jednoduchý stav nasadenia a obnovia iba služby, pri ktorých sa zmenili relevantné súbory, chýba kontajner alebo sa zmenil Docker image.
...
Evolúcia databázovej schémy vo vývojovom prostredí sa spolieha na manuálnu rekreáciu databázy a produkčné migrácie zatiaľ nie sú súčasťou automatizovaného nasadenia. Tieto zjednodušenia sú v aktuálnej fáze vývoja zámerné, avšak predstavujú jasne identifikované oblasti pre budúce zlepšenie s rastúcou vyspelosťou projektu.
